...
export default function ProfileScreen() {
    ...
    useEffect(() => {
        async function getAttempts() {
            ...
            // console.log(JSON.parse(result?.attempts));
        ...
        <SafeAreaView className='flex-1 bg-orange-100 p-4'>
            <View className='flex items-center justify-center mb-6'>
                ...
                <Pressable onPress={() => handleSignOut()} disabled={loading}>
                    <Text className='text-red-500'>{loading ? 'Logging out...' : 'Log out'}</Text>
                </Pressable>
            </View>

            <Text className='font-bold text-xl text-gray-700 mb-3 px-4'>Recent Attempts</Text>
            {dataLoading ? (
